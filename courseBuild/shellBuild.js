...
// const path = require('path'); // You may need to require the 'path' module
...
// const envFilePath = path.resolve(__dirname, '.env'); // Change '.env' to your actual .env file name if it's different
...
// const spreadsheetPath = process.env.SPREADSHEET_PATH;
// const coursePrefix = process.env.COURSE_PREFIX;
...
if (newPage) {
    console.log('Created Page:', newPage); // Log the newPage object to inspect its structure

    // Step 2: Find the module ID for "Course Overview"
    const moduleId = await findModuleIdByName(courseId, moduleName);
    if (moduleId) {
        // Step 3: Add the page to the "Course Overview" module
        // Ensure the correct URL/slug is used here
